...
export async function build(configPath = DEFAULT_CONFIG_PATH, selectors = [], modeName, featureToggles = {}, overrides = {}, resolvedConfig) {
    ...
    const duplicateSpecBasenames = await resolveAllConfiguredDuplicateBasenames(sourceInputPatterns);
...
function userSuppliesJsonAsTransform(config) {
    for (const raw of config.buildOptions.args) {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (token == "--transform") {
                ...
                if (value && JSON_AS_TRANSFORM_PATTERN.test(value))
                    return true;
            }
    ...
    if (config.config && config.config !== "none" && existsSync(config.config)) {
        if (asconfigDeclaresJsonAs(config.config, new Set()))
            return true;
    }
    return false;
}
function asconfigDeclaresJsonAs(configFile, seen) {
    ...
    if (!parsed || typeof parsed != "object")
        return false;
    const obj = parsed;
    if (transformsContainJsonAs(obj.options))
        return true;
    if (transformsContainJsonAs(obj))
        return true;
    const targets = obj.targets;
    if (targets && typeof targets == "object" && !Array.isArray(targets)) {
        for (const value of Object.values(targets)) {
            if (transformsContainJsonAs(value))
                return true;
        }
    }
