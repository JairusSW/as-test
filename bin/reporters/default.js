...
function renderCoverageSummary(summary, showCoverage) {
    ...
    const fileNameWidth = displayed.reduce((max, file) => Math.max(max, toRelativeResultPath(file.file).length), 0);
    for (const file of displayed) {
        ...
        console.log(`  ${fileColor(filePct.padStart(6) + "%")}  ${toRelativeResultPath(file.file).padEnd(fileNameWidth)} ${chalk.dim(`${file.covered}/${file.total} covered, ${suffix}`)}`);
    }
    if (ranked.length > 8) {
