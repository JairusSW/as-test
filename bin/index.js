...
function info() {
    ...
    console.log("  " +
        ...
        "                     Show help menu");
    console.log("");
    console.log(chalk.dim("If this tool provides value, please consider sponsoring my open-source work! https://github.com/sponsors/JairusSW") + "\n");
...
function printCommandHelp(command) {
    ...
    if (command == "test") {
        ...
        process.stdout.write("  --list                   Preview resolved files/artifacts/runtime without running\n");
        process.stdout.write("  --list-modes             Preview configured and selected mode names\n");
        process.stdout.write("  --watch                  Re-run on source or spec changes\n");
        process.stdout.write("  --help, -h               Show this help\n");
        return;
    }
    if (command == "fuzz") {
...
async function runTestSequential(runFlags, configPath, selectors, suiteSelectors, buildFeatureToggles, modeSummaryTotal, fileSummaryTotal, allowNoSpecFiles = false, modeName, reporterOverride, webSession = null, emitRunComplete = true) {
    ...
    const duplicateSpecBasenames = await resolveAllConfiguredDuplicateSpecBasenames(configPath);
...
async function runTestModesCore(runFlags, configPath, selectors, suiteSelectors, fuzzerSelectors, modes, buildFeatureToggles, fuzzEnabled, fuzzOverrides) {
    await ensureWebBrowsersReady(configPath, modes, runFlags.browser);
    const modeSummaryTotal = Math.max(modes.length, 1);
...
async function runTestModes(runFlags, configPath, selectors, suiteSelectors, fuzzerSelectors, modes, buildFeatureToggles, fuzzEnabled, fuzzOverrides) {
    if (runFlags.watch) {
        await runWatchLoop(runFlags, configPath, selectors, suiteSelectors, fuzzerSelectors, modes, buildFeatureToggles, fuzzEnabled, fuzzOverrides);
        return;
    }
    const failed = await runTestModesCore(runFlags, configPath, selectors, suiteSelectors, fuzzerSelectors, modes, buildFeatureToggles, fuzzEnabled, fuzzOverrides);
    process.exit(failed ? 1 : 0);
}
async function runWatchLoop(runFlags, configPath, selectors, suiteSelectors, fuzzerSelectors, modes, buildFeatureToggles, fuzzEnabled, fuzzOverrides) {
    const resolvedConfigPath = configPath ?? path.join(process.cwd(), "./as-test.config.json");
    ...
    async function doRun(changedFile) {
        ...
        try {
            await runTestModesCore(runFlags, configPath, selectors, suiteSelectors, fuzzerSelectors, modes, buildFeatureToggles, fuzzEnabled, fuzzOverrides);
        }
        catch (error) {
    ...
    for (const dir of watchDirs) {
        ...
        try {
            const w = fsWatch(dir, { recursive: true }, (_evt, filename) => {
                ...
                if (shouldIgnoreWatchPath(full, config))
                    return;
...
function shouldIgnoreWatchPath(filePath, config) {
    ...
    if (rel.startsWith("node_modules") || rel.startsWith(".git"))
        return true;
    const outRel = path.normalize(path.relative(cwd, path.resolve(cwd, config.outDir)));
    if (rel.startsWith(outRel + path.sep) || rel === outRel)
        return true;
    if (!rel.endsWith(".ts") && !rel.endsWith("as-test.config.json"))
        return true;
    return false;
}
...
async function listExecutionPlan(command, configPath, selectors, modes, listFlags, fuzzEnabled = false) {
    ...
    const duplicateSpecBasenames = await resolveAllConfiguredDuplicateSpecBasenames(configPath);
    const duplicateFuzzBasenames = await resolveAllConfiguredDuplicateFuzzBasenames(configPath);
    if (specFiles.length) {
