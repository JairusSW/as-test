...
describe("Should serialize and deserialize aliased keys", () => {
  ...
  expect(JSON.stringify(alias)).toBe(
    '{"first name":"Ada","last name":"Lovelace"}',
  );
  expect(
    JSON.stringify(
      JSON.parse<AliasEnvelope>('{"first name":"Ada","last name":"Lovelace"}'),
    ),
  ).toBe('{"first name":"Ada","last name":"Lovelace"}');
});

describe("Should preserve strange aliased keys across types", () => {
  expect(JSON.stringify<ObjWithStrangeKey<string>>({ data: "value" })).toBe(
    '{"a\\\\\\t\\"\\u0002b`c":"value"}',
  );
  expect(JSON.stringify<ObjWithStrangeKey<i32>>({ data: 42 })).toBe(
    '{"a\\\\\\t\\"\\u0002b`c":42}',
  );
  expect(JSON.stringify<ObjWithStrangeKey<bool>>({ data: true })).toBe(
    '{"a\\\\\\t\\"\\u0002b`c":true}',
  );
});
...
describe("Should apply decorator combinations on imported player type", () => {
  ...
  expect(JSON.stringify(minor)).toBe(
...
describe("Should deserialize decorated player payloads with omitted fields", () => {
  const parsedMinor = JSON.parse<FeaturePlayer>(
    '{"firstName":"Teen","lastName":"User","lastActive":[1,2,3],"isVerified":false}',
  );
