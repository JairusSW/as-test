...
describe("Should serialize JSON.Box<T>", () => {
  ...
  expect(
    JSON.stringify<JSON.Box<boolean> | null>(new JSON.Box<boolean>(false)),
  ).toBe("false");

  expect(
    JSON.stringify<JSON.Box<boolean> | null>(new JSON.Box<boolean>(true)),
  ).toBe("true");
});
...
describe("Should deserialize JSON.Box<T>", () => {
  ...
  expect(JSON.parse<JSON.Box<boolean> | null>("false")!.value.toString()).toBe(
    "false",
  );
