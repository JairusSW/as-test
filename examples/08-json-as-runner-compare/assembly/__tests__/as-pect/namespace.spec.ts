...
describe("Should deserialize namespaced object with alias property", () => {
  expect(
    JSON.stringify(
      JSON.parse<Namespace.ObjectWithAliasProperty>(`{"a":"foo","value":42}`),
    ),
  ).toBe(`{"a":"foo","value":42}`);
});
...
describe("Should deserialize namespaced derived structs", () => {
  expect(
    JSON.stringify(
      JSON.parse<Namespace.DerivedObject>(`{"a":"foo","b":"bar"}`),
    ),
  ).toBe(`{"a":"foo","b":"bar"}`);
  expect(
    JSON.stringify(
      JSON.parse<Namespace.DerivedObject>(`{"b":"bar","a":"foo"}`),
    ),
  ).toBe(`{"a":"foo","b":"bar"}`);
});
...
describe("Should deserialize namespaced derived structs with nested object", () => {
  expect(
    JSON.stringify(
      JSON.parse<Namespace.DerivedObjectWithNestedObject>(
        `{"a":"foo","b":"bar","c":{"value":"baz"}}`,
      ),
    ),
  ).toBe(`{"a":"foo","b":"bar","c":{"value":"baz"}}`);
  expect(
    JSON.stringify(
      JSON.parse<Namespace.DerivedObjectWithNestedObject>(
        `{"c":{"value":"baz"},"a":"foo","b":"bar"}`,
      ),
    ),
  ).toBe(`{"a":"foo","b":"bar","c":{"value":"baz"}}`);
});
...
describe("Should deserialize namespaced alias property", () => {
  const x = JSON.parse<Namespace.ObjectWithAliasProperty>(
    '{"a":"hello","value":42}',
  );
...
describe("Should round-trip namespaced arrays and nested objects", () => {
  expect(
    JSON.stringify(
      JSON.parse<Namespace.DerivedObject[]>(
        '[{"a":"x","b":"y"},{"a":"m","b":"n"}]',
      ),
    ),
  ).toBe('[{"a":"x","b":"y"},{"a":"m","b":"n"}]');

  const nested = JSON.parse<Namespace.DerivedObjectWithNestedObject>(
    '{"a":"root","b":"branch","c":{"value":"leaf"}}',
  );
