...
const CONFIG_META = new WeakMap<Config, ConfigMeta>();
...
function parseConfigRaw(
  ...
  config.buildOptions.env = parseEnvValue(
    (raw.buildOptions as Record<string, unknown> | undefined)?.env,
    configDir,
    "$.buildOptions.env",
  );
...
function validateCoverageField(
  raw: Record<string, unknown>,
  key: string,
  pathPrefix: string,
  issues: ValidationIssue[],
...
function validateCoverageValue(
  value: unknown,
  path: string,
  issues: ValidationIssue[],
): void {
...
function validateBuildOptionsField(
  raw: Record<string, unknown>,
  key: string,
  pathPrefix: string,
  issues: ValidationIssue[],
...
function validateRunOptionsField(
  raw: Record<string, unknown>,
  key: string,
  pathPrefix: string,
  issues: ValidationIssue[],
...
function validateModesField(
  ...
  for (const [modeName, modeRaw] of Object.entries(
...
function parseInlineEnvEntries(
  values: unknown[],
  pathLabel: string,
): Record<string, string> {
  const env: Record<string, string> = {};
...
function getConfigMeta(config: Config): ConfigMeta {
...
function cloneBuildOptions(options: BuildOptions): BuildOptions {
...
function cloneFuzzConfig(config: FuzzConfig): FuzzConfig {
  const cloned = Object.assign(new FuzzConfig(), config);
  cloned.input = [...config.input];
  return cloned;
}

function cloneModeConfig(config: ModeConfig): ModeConfig {
...
function mergeRootConfig(base: Config, override: Config): Config {
  ...
  if (
    ...
    merged.runOptions = mergeRunOptions(
      merged.runOptions,
      override.runOptions,
      raw.runOptions as Record<string, unknown>,
    );
  }
  if (raw.fuzz && typeof raw.fuzz == "object" && !Array.isArray(raw.fuzz)) {
    merged.fuzz = mergeFuzzConfig(
      merged.fuzz,
      override.fuzz,
      raw.fuzz as Record<string, unknown>,
    );
  }
...
function applyPerModeOutputDefaults(
  base: Config,
  merged: Config,
  override: Config,
  modeName: string,
...
function resolveModeOverrideConfig(root: Config, modeName: string): Config {
...
export type CacheMode = "off" | "build" | "full";
...
export type CacheSettings = { mode: CacheMode; maxTimeMs: number | null };
...
export function resolveCacheSettings(
  configCache: Config["cache"] | undefined,
  flags: { cache?: boolean; noCache?: boolean },
): CacheSettings {
