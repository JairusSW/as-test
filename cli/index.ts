...
import { type CacheMode, resolveCacheSettings } from "./util.js";
...
function expandShorthands(argv: string[]): string[] {
...
let changedSpecFilter: Set<string> | null = null;
...
function warnUnknownFlags(rawArgs: string[], command: string): void {
...
async function buildFileForMode(args: {
  ...
  if (args.buildPool) {
    ...
    const reads = cacheCtx && reuse ? new Set<string>() : null;
...
async function runWatchLoop(
  ...
  async function withCache(fn: () => Promise<void>): Promise<void> {
    if (cacheMode === "off") {
      await fn();
      return;
    }
...
function gitChangedFiles(): Set<string> | null {
...
async function activateChangedFilter(
  configPath: string | undefined,
): Promise<void> {
...
function aggregateRunResults(results: RunResult[]): {
  ...
  {
    const modePoints = new Map<string, Map<string, boolean>>();
    for (const result of results) {
      const modeName =
        (result.reports[0] as { modeName?: string } | undefined)?.modeName ??
        "default";
