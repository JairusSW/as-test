...
function specKey(mode: string | undefined, spec: string): string {
...
function isUninterestingDep(absPath: string): boolean {
...
export type DepFingerprint = { sha: string; mtimeMs: number; size: number };
...
type CacheEntry = {
  ...
  deps: Record<string, DepFingerprint>;
  // Epoch ms of the last actual build, for time-based expiry (maxTime).
  builtAt?: number;
  // Tier 2 replay only:
  runtimeCmd?: string | null;
  snapshotSha?: string | null;
  reportPath?: string | null; // relative to cacheDir
};

type Manifest = {
  version: number;
  asTestVersion: string;
  entries: Record<string, CacheEntry>;
};

export type FreshnessCtx = {
  signature: string;
  coverageEnabled: boolean;
};
...
export function resolveCacheDir(baseOutDir: string): string {
...
export class BuildCache {
  ...
  private constructor(
    ...
    private readonly asTestVersion: string,
    private manifest: Manifest,
    // Entry expiry window in ms (null = none) and the run's reference time.
    private readonly maxTimeMs: number | null,
  ...
  static load(
    cacheDir: string,
    asTestVersion: string,
    opts: { maxTimeMs?: number | null; now?: number } = {},
  ): BuildCache {
  ...
  canReplay(
    mode: string | undefined,
    spec: string,
    ctx: { runtimeCmd: string; snapshotSha: string | null },
  ): boolean {
    const entry = this.manifest.entries[specKey(mode, spec)];
  ...
  recordBuild(
    mode: string | undefined,
    spec: string,
    args: RecordBuildArgs,
  ): void {
    const absSpec = path.resolve(spec);
    const deps: Record<string, DepFingerprint> = {};
  ...
  getReport(mode: string | undefined, spec: string): unknown | undefined {
    const entry = this.manifest.entries[specKey(mode, spec)];
  ...
  keyFor(mode: string | undefined, spec: string): string {
  ...
  private fingerprint(absPath: string): DepFingerprint | null {
    try {
  ...
  private depUnchanged(absPath: string, fp: DepFingerprint): boolean {
    let st;
    try {
      st = statSync(absPath);
    } catch {
  ...
  private removeReportFile(relPath: string | null): void {
    if (!relPath) return;
    try {
...
export function reportHasFailure(report: unknown): boolean {
  const suites = (report as { suites?: unknown[] })?.suites;
  if (!Array.isArray(suites)) return false;
  return suites.some(suiteHasFailure);
}

function suiteHasFailure(suite: unknown): boolean {
  const s = suite as {
    verdict?: string;
    suites?: unknown[];
    tests?: { verdict?: string }[];
  };
