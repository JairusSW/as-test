...
function tryLoadConfig(configPath: string): { loaded: boolean; config: Config } {
  try {
...
async function checkMode(
  ...
  const checks: CheckResult[] = [];
  let applied: ReturnType<typeof applyMode>;
  try {
...
async function checkInputPatterns(
  input: string[] | string,
  scope: string,
): Promise<CheckResult[]> {
...
function extractRuntimeScriptPath(args: string[]): string | null {
...
function isLikelyScriptPath(token: string): boolean {
